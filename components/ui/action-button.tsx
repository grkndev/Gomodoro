...
const actionButtonVariants = cva(
  ...
  ),
  {
    variants: {
      variant: {
        default: cn(
          'bg-[#17CB17] border-b-4 border-[#26FF26]',
      ...
      size: {
        default: cn('px-9 py-4 w-full group shrink-0 flex-row items-center justify-center gap-2 rounded-full shadow-none ', Platform.select({ web: 'has-[>svg]:px-3' })),
...
function ActionButton({ className, variant, size, ...props }: ActionButtonProps) {
  return (
    <TextClassContext.Provider value={actionButtonTextVariants({ variant, size })}>
      <Pressable
        className={cn(props.disabled && 'opacity-50', actionButtonVariants({ variant, size }), className)}
        role="button"
        {...props}
      />
